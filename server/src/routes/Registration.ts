...
registrationRouter.get("/", async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    return res.status(400).send("Error getting registrations.");
  }
});
...
registrationRouter.get("/:id", async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(generateResultJson(registration));
  } catch (error) {
    console.error(error);
    return res.status(400).send("Error getting registration.");
  }
});
...
registrationRouter.post("/", async (req, res) => {
  ...
  try {
    ...
    registration_details.subscription_model_id = Number(
      registration_details.subscription_model
    );
    registration_details.company_name =
      registration_details.company_name.toUpperCase();
    registration_details.email = registration_details.email.toUpperCase();
    registration_details.registrant_name =
      registration_details.registrant_name.toUpperCase();
    ...
    return res.status(200).json(generateResultJson(new_registration));
  } catch (error) {
    console.error(error);
...
registrationRouter.post("/update", async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(generateResultJson(updated_registration));
  } catch (error) {
    console.error(error);
    return res.status(400).send(error);
  }
...
const approveRegistration = async (registration_details: Registration) => {
  try {
    // do all data creation in one transaction
    return await prisma.$transaction(async (tx) => {
      // check if company uen already exists
      const existing_company = await tx.company.findFirst({
        where: {
      ...
      const subscription_model = await tx.subscriptionModel.findFirst({
        where: {
      ...
      await tx.companyCodeType.createMany({
        data: code_types,
      });
      ...
      const system_admin = await tx.user.create({
